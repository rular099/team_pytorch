...
# Submit waveform-station mismatch evaluation for rt44-rt51 KNET-only runs.
...
JOB_NAME_PREFIX=${JOB_NAME_PREFIX:-team-eval-knet-mismatch}
...
for rt in $RT_LIST; do
    ...
    echo "[INFO] submitting rt${rt} KNET-only waveform-station mismatch eval"
    WORKDIR="$WORKDIR" \
