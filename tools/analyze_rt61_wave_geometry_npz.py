#!/usr/bin/env python3
"""Reproduce the formal RT61 random/normal validation decision.

The RT61 candidate, its same-forward immutable RT59 reference, and the
...
pre-registered RT61 mechanism gates independently from the legacy gates.
"""
...
def _load_metrics_identity(path, expected_source_manifest_sha256):
    path = Path(path)
    payload = json.loads(path.read_text())
    protocol = payload.get('metric_protocol') or {}
    metadata = payload.get('checkpoint_metadata') or {}
    reference = metadata.get('rt61_reference_identity') or {}
    trainable_manifest = metadata.get('rt61_trainable_manifest') or {}
    ...
    checks = {
        ...
        'source_manifest': (
            source.get('rt61_uploaded_source_manifest_sha256')
            == expected_source_manifest_sha256
        ),
        'no_heldout_test_contract': source.get('split_contract')
        == 'inherited full 2000-2024 train/dev; no held-out test',
        'trainable_tensor_count': trainable_manifest.get('tensor_count') == 9,
        'trainable_scalar_count': trainable_manifest.get('scalar_count') == 99621,
        'frozen_shared_state_verified': metadata.get(
            'rt61_frozen_shared_state_verified_unchanged'
        ) is True,
        'frozen_shared_fingerprint_matches_parent': metadata.get(
            'rt61_frozen_shared_state_fingerprint'
        ) == reference.get('parent_shared_fingerprint'),
    }
    ...
    if not normalization.get('enabled'):
        raise ValueError('RT61 normalization metadata is absent or disabled.')
    mean = float(normalization['mean'])
    std = float(normalization['std'])
    if not np.isfinite([mean, std]).all() or std <= 0:
        raise ValueError('invalid RT61 normalization metadata.')
    return {
        'file': path.name,
...
def _load_rt61_protocol(npz_path, metrics_identity):
    ...
    valid = legacy['valid']
    reference = _target_matrix(_stack(npz, 'val_rt61_reference_mean'))
    increment = _target_matrix(_stack(npz, 'val_rt61_increment'))
    reference_mdn = np.asarray(_stack(npz, 'val_rt61_reference_mdn'), dtype=np.float64)
    adapter_off = _target_matrix(_stack(npz, 'val_rt61_adapter_off_mean'))
    adapter_off_increment = _target_matrix(
        _stack(npz, 'val_rt61_adapter_off_increment')
    )
    adapter_off_mdn = np.asarray(
        _stack(npz, 'val_rt61_adapter_off_mdn'), dtype=np.float64
    )
    ...
    magnitude = _optional(npz, 'val_mag_label')
    if magnitude is not None:
        magnitude = np.asarray(magnitude, dtype=np.float64).reshape(
            valid.shape[0], -1
        )[:, 0]
    query_coords = np.asarray(
        _stack(npz, 'val_pga_target_abs'), dtype=np.float64
    )
    if query_coords.shape[:2] != valid.shape or query_coords.shape[-1] < 2:
        raise ValueError('RT61 query-coordinate export is not aligned to targets.')

    models = {}
    for name, mdn, exported_mean in (
        ('historical_rt57', historical_mdn, legacy['base']),
        ('reference_rt59', reference_mdn, reference),
        ('adapter_off_counterfactual', adapter_off_mdn, adapter_off),
        ('candidate_rt61', candidate_mdn, legacy['final']),
    ):
    ...
    if reconstruction_error > 1e-6:
        raise ValueError(
            'RT61 reference + increment does not reconstruct candidate: '
            f'{reconstruction_error:.9g}'
        )
    adapter_off_reconstruction_error = float(np.max(np.abs(
        reference[valid] + adapter_off_increment[valid] - adapter_off[valid]
    )))
    if adapter_off_reconstruction_error > 1e-6:
        raise ValueError(
            'RT61 reference + adapter-off increment does not reconstruct the '
            f'counterfactual: {adapter_off_reconstruction_error:.9g}'
        )
    ...
    for name, value in exported.items():
        error = float(np.max(np.abs(value[valid] - models['candidate_rt61'][name][valid])))
    ...
    component_identity = {
        ...
        'predictive_sigma_max_abs_difference': float(np.max(np.abs(
            models['candidate_rt61']['sigma'][valid]
            - models['reference_rt59']['sigma'][valid]
        ))),
    }
    component_identity.update({
        'adapter_off_logit_max_abs_difference': float(np.max(np.abs(
            adapter_off_mdn[..., 0][valid] - reference_mdn[..., 0][valid]
        ))),
        'adapter_off_component_sigma_max_abs_difference': float(np.max(np.abs(
            adapter_off_mdn[..., 2][valid] - reference_mdn[..., 2][valid]
        ))),
    })
    if max(component_identity.values()) > 1e-6:
        raise ValueError(f'RT61 changed non-mean MDN quantities: {component_identity}')

    return {
        **legacy,
        'models': models,
        'increment': increment,
        'adapter_off_increment': adapter_off_increment,
        'magnitude': magnitude,
        'query_coords': query_coords,
        'identity': {
            'candidate_reconstruction_max_abs_error': reconstruction_error,
            'adapter_off_reconstruction_max_abs_error': (
                adapter_off_reconstruction_error
            ),
...
def _population(data, mask, seed, draws):
    ...
    for baseline in ('reference_rt59', 'historical_rt57'):
        ...
        candidate_brier = (
            data['models']['candidate_rt61']['probability'] - truth_binary
        ) ** 2
...
def _field_records(data, mask):
    ...
    for row in range(mask.shape[0]):
        ...
        for name, model in data['models'].items():
            ...
            prefix = name.replace('_rt57', '').replace('_rt59', '').replace('_rt61', '')
            record.update({
...
def _field_error_decomposition(protocol, data, mask):
    """Per-field absolute/centered error and truth-oracle shape projection."""
    mask = np.asarray(mask, dtype=bool) & data['valid']
    records = []
    for row in range(mask.shape[0]):
        valid = np.flatnonzero(mask[row])
        if valid.size < 2:
            continue
        truth = data['target'][row, valid].astype(np.float64)
        truth_centered = truth - truth.mean()
        pairs = np.triu_indices(valid.size, 1)
        for model_name, model in data['models'].items():
            prediction = model['mean'][row, valid].astype(np.float64)
            error = prediction - truth
            mean_error = float(error.mean())
            centered_error_mse = float(np.mean((error - mean_error) ** 2))
            field_mse = float(np.mean(error ** 2))
            prediction_centered = prediction - prediction.mean()
            denominator = float(np.dot(prediction_centered, prediction_centered))
            identifiable = denominator > 1e-12
            if identifiable:
                a_star = float(
                    np.dot(prediction_centered, truth_centered) / denominator
                )
                orthogonal_error = float(np.mean(
                    (truth_centered - a_star * prediction_centered) ** 2
                ))
                nonnegative_a_star = max(a_star, 0.0)
                nonnegative_orthogonal_error = float(np.mean(
                    (truth_centered
                     - nonnegative_a_star * prediction_centered) ** 2
                ))
            else:
                a_star = None
                orthogonal_error = None
                nonnegative_a_star = None
                nonnegative_orthogonal_error = None
            pred_delta = (
                prediction[pairs[0]] - prediction[pairs[1]]
            )
            truth_delta = truth[pairs[0]] - truth[pairs[1]]
            records.append({
                'protocol': protocol,
                ...
                'n_targets': int(valid.size),
                'model': model_name,
                'field_mean_error': mean_error,
                'field_mean_error_squared': mean_error ** 2,
                'centered_error_mse': centered_error_mse,
                'field_mse': field_mse,
                'mse_decomposition_abs_error': abs(
                    field_mse - mean_error ** 2 - centered_error_mse
                ),
                'truth_p95_p05_range': _canonical_field_range(truth),
                'prediction_p95_p05_range': _canonical_field_range(prediction),
                'pairwise_delta_mae': float(np.mean(np.abs(
                    pred_delta - truth_delta
                ))),
                'projection_identifiable': identifiable,
                'a_star_signed_oracle': a_star,
                'orthogonal_error_oracle': orthogonal_error,
                'a_star_nonnegative_oracle': nonnegative_a_star,
                'orthogonal_error_nonnegative_oracle': (
                    nonnegative_orthogonal_error
                ),
                'oracle_warning': (
                    'offline_truth_only_not_deployable'
                    if identifiable else 'prediction_near_constant_undefined'
                ),
            })
    return records
...
def _strata_rows(protocol, data):
    ...
    if data.get('magnitude') is not None:
        ...
        for lower, upper in zip(edges[:-1], edges[1:]):
            ...
            field_mask = (
                (data['magnitude'] >= lower)
                & (data['magnitude'] < upper)
            )
            definitions.append((
                'catalog_magnitude', label,
                np.broadcast_to(field_mask[:, None], data['valid'].shape),
            ))
    pga_edges = (-np.inf, -2.0, -1.5, -1.0, -0.5, np.inf)
    for lower, upper in zip(pga_edges[:-1], pga_edges[1:]):
        definitions.append((
            'truth_pga_log10_mps2', f'[{lower:g},{upper:g})',
            (data['target'] >= lower) & (data['target'] < upper),
        ))
    rows = []
    for stratum, value, mask in definitions:
        ...
        for model_name, model in data['models'].items():
            prefix = model_name.replace('_rt57', '').replace('_rt59', '').replace('_rt61', '')
            metrics = _point_metrics(data['target'], model['mean'], mask)
...
def _training_summary(directory):
    filenames = {
        ...
        'rt61_total': 'rt61_epoch_loss_total.csv',
        'point': 'rt61_epoch_loss_point.csv',
        'contrast': 'rt61_epoch_loss_contrast.csv',
        'regret': 'rt61_epoch_loss_regret.csv',
        'nll': 'rt61_epoch_loss_nll.csv',
        'smooth': 'rt61_epoch_loss_smooth.csv',
        'mse': 'rt61_epoch_loss_mse.csv',
        'lr_trainable': 'rt61_epoch_lr_trainable.csv',
        'grad_pre_clip': 'rt61_epoch_trainable_pre_clip_norm.csv',
        'grad_post_clip': 'rt61_epoch_trainable_post_clip_norm.csv',
        'readout_grad_pre_clip': 'rt61_epoch_readout_pre_clip_norm.csv',
        'readout_grad_post_clip': 'rt61_epoch_readout_post_clip_norm.csv',
        'adapter_grad_pre_clip': 'rt61_epoch_adapter_pre_clip_norm.csv',
        'adapter_grad_post_clip': 'rt61_epoch_adapter_post_clip_norm.csv',
        'readout_parameter_delta_l2': 'rt61_epoch_readout_parameter_delta_l2.csv',
        'adapter_parameter_delta_l2': 'rt61_epoch_adapter_parameter_delta_l2.csv',
        'targets_random': 'rt61_epoch_targets_random.csv',
        'targets_normal_observed': 'rt61_epoch_targets_normal_observed.csv',
        'targets_normal_remote': 'rt61_epoch_targets_normal_remote.csv',
        'fields_random_single': 'rt61_epoch_fields_random_single.csv',
        'fields_random_multi': 'rt61_epoch_fields_random_multi.csv',
        'fields_normal_remote_single': 'rt61_epoch_fields_normal_remote_single.csv',
        'fields_normal_remote_multi': 'rt61_epoch_fields_normal_remote_multi.csv',
    }
    series = {name: _read_epoch_csv(directory, filename) for name, filename in filenames.items()}
    expected_lr = [1e-4] * 4 + [5e-5] * 2 + [2.5e-5] * 2
    if not np.allclose(series['lr_trainable'], expected_lr, rtol=0.0, atol=1e-12):
        raise ValueError(f'RT61 LR schedule mismatch: {series["lr_trainable"]}')
    rows = []
...
def _validate_training_config(path, expected_source_manifest_sha256):
    ...
    checks = {
        ...
        'rt61_model_enabled': model.get('use_rt61_wave_geometry_adapter') is True,
        'rt61_rank16': model.get('rt61_wave_geometry_rank') == 16,
        'rt60_model_disabled': not model.get('use_rt60_contrast_readout', False),
        'freeze_mode': training.get('freeze_mode') == 'rt61_wave_geometry_residual_only',
        ...
        'rt61_objective_enabled': training.get('rt61_contrast_objective', {}).get('enabled') is True,
        'parent_task': training.get('rt61_parent', {}).get('task_id') == PARENT_TASK_ID,
        'parent_epoch8': training.get('rt61_parent', {}).get('epoch') == 8,
        'source_manifest': training.get('rt61_parent', {}).get('source_identity', {}).get(
            'rt61_uploaded_source_manifest_sha256'
        ) == expected_source_manifest_sha256,
    }
    failed = [name for name, passed in checks.items() if not passed]
    if failed:
...
def _select_fixed_examples(records):
    eligible = [record for record in records if record['eligible_ge5']]
    if not eligible:
        raise ValueError('no eligible fields for fixed example selection.')
    ranked = sorted(eligible, key=lambda record: (
        record['candidate_pairwise_delta_mae']
        - record['reference_pairwise_delta_mae'],
        record['row_index'],
    ))
    deltas = np.asarray([
        record['candidate_pairwise_delta_mae']
    ...
    selected = [
        ('largest_improvement', ranked[0]),
        ('median_change', middle),
        ('largest_degradation', ranked[-1]),
    ]
    return [{
        'selection_rule': label,
        'row_index': record['row_index'],
        'event_id': record['event_id'],
        'requested_elapsed_time_seconds': record[
            'requested_elapsed_time_seconds'
        ],
        'actual_station_count': record['actual_station_count'],
        'n_targets': record['n_targets'],
        'candidate_minus_reference_pairwise_delta_mae': (
            record['candidate_pairwise_delta_mae']
            - record['reference_pairwise_delta_mae']
        ),
...
def _write_fixed_examples_figure(path, data, selected):
    try:
        import matplotlib
        matplotlib.use('Agg')
    ...
    for item in selected:
        ...
        arrays.extend([
            data['target'][row, valid],
            data['models']['reference_rt59']['mean'][row, valid],
            data['models']['candidate_rt61']['mean'][row, valid],
        ])
    ...
    for row_axis, item in enumerate(selected):
        row = int(item['row_index'])
        valid = data['valid'][row]
        coords = data['query_coords'][row, valid]
        panels = (
            ('Truth', data['target'][row, valid]),
            ('RT59 reference', data['models']['reference_rt59']['mean'][row, valid]),
            ('RT61 candidate', data['models']['candidate_rt61']['mean'][row, valid]),
        )
        for column, (title, values) in enumerate(panels):
            axis = axes[row_axis, column]
...
def main():
    ...
    if args.bootstrap_draws != 5000 or args.bootstrap_seed != BOOTSTRAP_SEED:
        raise ValueError('formal RT61 decision requires 5000 draws and seed 20260915.')
    if (
        len(args.expected_source_manifest_sha256) != 64
        or any(char not in '0123456789abcdefABCDEF'
               for char in args.expected_source_manifest_sha256)
    ):
        raise ValueError('expected source manifest must be a full SHA-256.')
    ...
    analysis_provenance = {
        ...
        'rt61_implementation_commit': args.implementation_commit,
        'rt61_submitted_source_manifest_sha256': (
            args.expected_source_manifest_sha256.lower()
        ),
    }
    ...
    random_identity = _load_metrics_identity(
        args.random_metrics, args.expected_source_manifest_sha256.lower()
    )
    normal_identity = _load_metrics_identity(
        args.normal_metrics, args.expected_source_manifest_sha256.lower()
    )
    ...
    if random_identity['reference_identity'] != normal_identity['reference_identity']:
        raise ValueError('random/normal RT61 reference identities disagree.')
    if random_identity['trainable_manifest'] != normal_identity['trainable_manifest']:
        raise ValueError('random/normal RT61 trainable manifests disagree.')
    random_data = _load_rt61_protocol(args.random_npz, random_identity)
    normal_data = _load_rt61_protocol(args.normal_npz, normal_identity)
    ...
    fixed_examples = _select_fixed_examples(random_records)
    ...
    def point_delta(population, metric, baseline='reference_rt59'):
        ...
        return models['candidate_rt61'][metric] - models[baseline][metric]
    ...
    if len(mechanism_gates) != 14:
        raise AssertionError('RT61 mechanism gate inventory must contain 14 gates.')
    ...
    legacy_pair = 'candidate_rt61_minus_historical_rt57'
    legacy_gates = [
        _gate('random_mae', groups['random']['models']['candidate_rt61']['mae'], 0.242, '<='),
        _gate('random_slope', groups['random']['models']['candidate_rt61']['slope'], 0.45, '>='),
        _gate('random_r2', groups['random']['models']['candidate_rt61']['r2'], 0.39, '>='),
        ...
        _gate('random_requested1_mae', requested1['models']['candidate_rt61']['mae'], 0.253, '<='),
        _gate('random_nll', groups['random']['models']['candidate_rt61']['nll'], 0.22, '<='),
    ...
    legacy_gates.extend([
        _gate('normal_all_mae_historical_redline', groups['normal_all']['models']['candidate_rt61']['mae'], 0.136, '<='),
        _gate('normal_noninput_mae_historical_redline', groups['normal_noninput']['models']['candidate_rt61']['mae'], 0.218, '<='),
    ...
    diagnostics = []
    for population in ('normal_all', 'normal_noninput'):
        candidate = groups[population]['models']['candidate_rt61']
    ...
    reference_pairwise_mae = field_summary['one_station']['reference'][
        'pairwise_delta_mae'
    ]
    candidate_pairwise_mae = field_summary['one_station']['candidate'][
        'pairwise_delta_mae'
    ]
    useful_threshold = 0.98 * reference_pairwise_mae
    reference_pair = 'candidate_rt61_minus_reference_rt59'
    useful_gates = [
        {
            'gate': 'mechanism_pass',
            'value': mechanism_pass,
            'rule': 'is True',
            'pass': mechanism_pass,
            'required': True,
        },
        _gate(
            'one_station_pairwise_delta_mae_2pct_improvement',
            candidate_pairwise_mae,
            useful_threshold,
            '<=',
        ),
        _gate(
            'normal_noninput_mae_paired_ci_upper',
            groups['normal_noninput']['paired'][reference_pair]['mae']['upper'],
            0.0,
            '<',
        ),
        _gate(
            'normal_noninput_rmse_paired_ci_upper',
            groups['normal_noninput']['paired'][reference_pair]['rmse']['upper'],
            0.0,
            '<',
        ),
    ]
    useful_joint_progress = all(item['pass'] is True for item in useful_gates)

    training = _training_summary(args.training_log_dir)
    training_config_contract = _validate_training_config(
        args.training_config, args.expected_source_manifest_sha256.lower()
    )
    config_sources = {}
    for label, source in (
    ...
    field_error_decomposition = (
        _field_error_decomposition('random', random_data, random_mask)
        + _field_error_decomposition(
            'normal_noninput', normal_data, normal_noninput
        )
    )
    maximum_mse_decomposition_error = max(
        record['mse_decomposition_abs_error']
        for record in field_error_decomposition
    )
    if maximum_mse_decomposition_error > 1e-10:
        raise ValueError(
            'field MSE decomposition identity failed: '
            f'{maximum_mse_decomposition_error:.9g}'
        )
    ...
    summary = {
        ...
        'rt61_mechanism_pass': mechanism_pass,
        'useful_joint_progress': useful_joint_progress,
        'legacy_full_go': legacy_full_go,
        'recommendation': (
            'rt61_ready_for_preregistered_independent_validation'
            if useful_joint_progress else 'retain_rt59_reference'
        ),
        ...
        'identity_checks': {
            ...
            'shared_state_fingerprint': random_identity['reference_identity'].get(
                'parent_shared_fingerprint'
            ),
            'immutable_readout_sha256': random_identity['reference_identity'].get(
                'readout_sha256'
            ),
        },
        'groups': groups,
        ...
        'useful_joint_progress_gates': useful_gates,
        'useful_joint_progress_thresholds': {
            'reference_one_station_pairwise_delta_mae_full_precision': (
                reference_pairwise_mae
            ),
            'candidate_max_for_2pct_improvement': useful_threshold,
        },
    ...
    _write_rows(output_dir / 'useful_joint_progress_gates.csv', useful_gates)
    _write_rows(output_dir / 'legacy_gates.csv', legacy_gates + diagnostics)
    _write_rows(output_dir / 'field_metrics.csv', random_records)
    _write_rows(
        output_dir / 'strata_metrics.csv',
        _strata_rows('random', random_data)
        + _strata_rows('normal', normal_data),
    )
    _write_rows(output_dir / 'training_summary.csv', training['epochs'])
    _write_rows(
        output_dir / 'field_error_decomposition.csv',
        field_error_decomposition,
    )
    _write_rows(output_dir / 'fixed_examples.csv', fixed_examples)
    ...
    reference_invariance = {
        ...
        'field_mse_decomposition_max_abs_error': (
            maximum_mse_decomposition_error
        ),
    }
    (output_dir / 'reference_invariance.json').write_text(
        json.dumps(reference_invariance, indent=2, sort_keys=True) + '\n'
    )
    (output_dir / 'trainable_manifest.json').write_text(
        json.dumps(
            random_identity['trainable_manifest'], indent=2, sort_keys=True
        ) + '\n'
    )
    decision = {
        'task_id': TASK_ID,
    ...
    summary['fixed_examples_figure_written'] = _write_fixed_examples_figure(
        output_dir / 'fixed_examples.png', random_data, fixed_examples
    )
    (output_dir / 'summary.json').write_text(
        json.dumps(summary, indent=2, sort_keys=True) + '\n'
    )
    ...
    readme.extend(['', '## Useful joint progress gates', '', '| Gate | Value | Rule | Pass |', '|---|---:|---:|:---:|'])
    for item in useful_gates:
        readme.append(f"| {item['gate']} | {item['value']} | {item['rule']} | {item['pass']} |")
    ...
    review_text = '\n'.join(readme) + '\n'
    (output_dir / 'README.md').write_text(review_text)
    (output_dir / 'RESULT_REVIEW.md').write_text(review_text)
    manifest_lines = []
    for artifact in sorted(output_dir.iterdir()):
        if artifact.is_file() and artifact.name != 'artifact_manifest.sha256':
            manifest_lines.append(f'{_sha256_file(artifact)}  {artifact.name}')
