...
# RT61: one fixed eight-epoch residual-readout plus rank-16 adapter train from
# the exact RT59 epoch-8
...
CONFIG=${CONFIG:-$WORKDIR/pga_configs/transformer_japan_full_2000_2024_rt61_wave_geometry_residual_seed42_chaosuan.json}
NORMAL_CONFIG=${NORMAL_CONFIG:-$WORKDIR/pga_configs/transformer_japan_full_2000_2024_rt61_wave_geometry_residual_seed42_normal_validation_chaosuan.json}
...
RT59_PARENT_CHECKPOINT_SHA256=${RT59_PARENT_CHECKPOINT_SHA256:-5dbc15c6af5b8d341dfd4a377a68217b1e1aa3589550690997c02e81332aa1cd}
RT61_WEIGHT_PATH=${RT61_WEIGHT_PATH:-$WORKDIR/weights_japan_full_2000_2024_rt61_wave_geometry_residual_seed42}

ACTION=${ACTION:-all}
DRY_RUN=${DRY_RUN:-1}
CONFIRM_RT61=${CONFIRM_RT61:-0}
...
TRAIN_JOB_NAME=${TRAIN_JOB_NAME:-team-rt61-wave-geometry-train}
RANDOM_JOB_NAME=${RANDOM_JOB_NAME:-team-rt61-random-val}
NORMAL_JOB_NAME=${NORMAL_JOB_NAME:-team-rt61-normal-val}
...
if [[ "$DRY_RUN" != "1" && "$CONFIRM_RT61" != "1" ]]; then
    echo "RT61 submission requires CONFIRM_RT61=1 (or keep DRY_RUN=1)." >&2
    exit 2
fi
...
SOURCE_MANIFEST_FILES=(
    ...
    tools/rt59_dual_objective.py tools/rt60_contrast_objective.py tools/rt61_wave_geometry.py
    tools/analyze_rt59_dual_objective_npz.py tools/analyze_rt61_wave_geometry_npz.py
    tools/run_rt61_wave_geometry_slurm.sh
    ...
    pga_configs/transformer_japan_full_2000_2024_rt61_wave_geometry_residual_seed42_chaosuan.json
    pga_configs/transformer_japan_full_2000_2024_rt61_wave_geometry_residual_seed42_normal_validation_chaosuan.json
)
...
case "$SOURCE_IDENTITY_MODE" in
    git)
        ...
        [[ -z "$dirty" ]] || { echo "RT61 requires a clean Git worktree:" >&2; printf '%s\n' "$dirty" >&2; exit 1; }
...
RT61_SOURCE_MANIFEST_SHA256=$actual_manifest

for required in "$CONFIG:RT61 config" "$NORMAL_CONFIG:RT61 normal config" "$TRAIN_SCRIPT:training launcher" "$EVAL_SCRIPT:evaluation launcher"; do
...
if [[ "$DRY_RUN" != "1" ]]; then
    [[ "$RT59_PARENT_CHECKPOINT_SHA256" =~ ^[0-9a-fA-F]{64}$ ]] || { echo "Real RT61 work requires RT59_PARENT_CHECKPOINT_SHA256." >&2; exit 2; }
...
[[ "$RT59_PARENT_CHECKPOINT_SHA256" == "5dbc15c6af5b8d341dfd4a377a68217b1e1aa3589550690997c02e81332aa1cd" ]] || {
    echo "RT61 is preregistered to the exact RT59 epoch-8 parent SHA-256." >&2
    exit 1
}
...
case "$RT61_WEIGHT_PATH" in /*) RT61_WEIGHT_DIR=$RT61_WEIGHT_PATH ;; *) RT61_WEIGHT_DIR=$WORKDIR/${RT61_WEIGHT_PATH#./} ;; esac
if [[ -z "$RT61_WEIGHT_DIR" || "$RT61_WEIGHT_DIR" == "/" || "$RT61_WEIGHT_DIR" == "$WORKDIR" ]]; then echo "Unsafe RT61 weight directory: $RT61_WEIGHT_DIR" >&2; exit 1; fi
RT61_EPOCH8_CHECKPOINT=$RT61_WEIGHT_DIR/full_model_last.pth
RT61_LOG_NAME=$(basename "$RT61_WEIGHT_DIR")
if [[ "$ACTION" == "train" || "$ACTION" == "all" ]]; then
    if [[ -d "$RT61_WEIGHT_DIR" ]]; then
        entry_count=$(find "$RT61_WEIGHT_DIR" -maxdepth 1 -mindepth 1 -printf x | wc -c)
        [[ "$entry_count" == "0" ]] || { echo "RT61 training directory must be new and empty: $RT61_WEIGHT_DIR" >&2; exit 1; }
    fi
else
    require_file "$RT61_EPOCH8_CHECKPOINT" "RT61 epoch-8 checkpoint"
fi

RANDOM_LOG_DIR=${RANDOM_LOG_DIR:-$WORKDIR/logs/$RT61_LOG_NAME/epoch8_random_validation}
NORMAL_LOG_DIR=${NORMAL_LOG_DIR:-$WORKDIR/logs/$RT61_LOG_NAME/epoch8_normal_validation}
...
export RT59_PARENT_CHECKPOINT_SHA256 RT61_WEIGHT_PATH RT61_SOURCE_MANIFEST_SHA256

echo "[INFO] action=$ACTION dry_run=$DRY_RUN source_mode=$SOURCE_IDENTITY_MODE"
echo "[INFO] source_manifest_sha256=$RT61_SOURCE_MANIFEST_SHA256"
echo "[INFO] RT59 parent=$RT59_PARENT_CHECKPOINT sha256=$actual_parent_sha"
echo "[INFO] RT61 checkpoint=$RT61_EPOCH8_CHECKPOINT"
echo "[INFO] protocol=seed42, 8 new epochs, 9 trainable tensors (count verified at runtime), train -> random/normal val; no test/smoke/sweep"
if [[ "$DRY_RUN" == "1" ]]; then
...
if [[ "$ACTION" == "train" || "$ACTION" == "all" ]]; then
    ...
    [[ "$train_job_id" =~ ^[0-9]+$ ]] || { echo "Could not parse RT61 training job ID; validation was not submitted." >&2; exit 1; }
fi
...
submit_eval() {
    ...
    output=$(WORKDIR="$WORKDIR" JOB_NAME="$name" SLURM_PARTITION="$SLURM_PARTITION" \
        ...
        EVAL_CHECKPOINT="$RT61_EPOCH8_CHECKPOINT" EVAL_OUTPUT_TXT="$stem.txt" \
...
if [[ "$ACTION" == "eval" || "$ACTION" == "all" ]]; then
    ...
    [[ "$random_job_id" =~ ^[0-9]+$ && "$normal_job_id" =~ ^[0-9]+$ ]] || { echo "Could not parse RT61 validation job IDs." >&2; exit 1; }
    echo "[OK] validation jobs: random=$random_job_id normal=$normal_job_id"
fi
echo "[OK] requested RT61 jobs submitted."
