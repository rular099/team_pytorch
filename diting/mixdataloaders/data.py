...
# the original waveforms are in 40 Hz of shape (4800, 3), we need to resample to 100 Hz of shape (12000, 3)
...
def get_from_DiTing_100Hz(part=None, key=None, h5file_path=''):
    """
    get waveform from DiTing Dataset
    TODO: description
    """
...
def get_from_MLAAPDE(key=None, h5file_path=''):
    """
    get waveform from MLAAPDE Dataset
    TODO: description
    """
    ...
    with h5py.File(h5file_path + cur_h5path, 'r') as f:
        # get the waveforms
...
def get_from_CEA09_20(key=None, h5file_path=''):
    """
    get waveform from MLAAPDE Dataset
    TODO: description
    """
...
def get_from_CREDITX1(key=None, h5file_path=''):
    """
    get waveform from CREDITX1 Dataset
    TODO: description
    """
    with h5py.File(h5file_path, 'r') as f:
        t_Length = min([int(f[key]['BHZ'].attrs['waveform_length_sample']), int(f[key]['BHN'].attrs['waveform_length_sample']), int(f[key]['BHE'].attrs['waveform_length_sample'])])
        
        waveforms = np.zeros((int(t_Length), 3), dtype=np.float32)
        waveforms[:, 0] = f[key]['BHZ'][()][:t_Length]
        waveforms[:, 1] = f[key]['BHN'][()][:t_Length]
        waveforms[:, 2] = f[key]['BHE'][()][:t_Length]
...
def get_from_CSNCD(key, h5_file_path):
    key_splits = key.split('|')
    cur_h5_path = key_splits[0]
    ev_key = key_splits[1]
    sta_key = key_splits[2]
    t_Instrument = key_splits[3]
    cur_h5_name = h5_file_path + cur_h5_path
    try:
        with h5py.File(cur_h5_name, 'r') as f:
...
def get_CEA_YZY_Data(key_set, h5file_path=''):
    """
    get waveform from CEA YZY
    """
...
def get_shifted_instance_for_EqDetPhasePicking_training(dataset_name = 'DiTing',
    ...
    elif dataset_name == 'DiTing_100Hz':
        try:
            data = get_from_DiTing_100Hz(part=part, key=key, h5file_path=dataset_path)
        except:
            print('Error on key {} DiTing'.format(key))
            return temp_data_X, temp_data_Y       
    ...
    if ori_length < data_length:
        ...
        if S > data_length and (P - 200 + data_length) < ori_length:
...
def get_augmented_instance_for_EqDetPhasePicking_training(dataset_name='DiTing',
    ...
    for key_dx, key in enumerate(key_list):
        ...
        if dataset_name == 'DiTing':
            ...
            try:
                data = get_from_DiTing_100Hz(part=temp_part_list[key_dx], key=key, h5file_path=dataset_path)
            except:
                print('Error on key {} DiTing'.format(key))
                continue
        ...
        for chn_dx in range(3):
            ...
            if dataset_name == 'DiTing' or 'DiTing_100Hz':
...
class DiTingGenerator:
    ...
    def __init__(self, csv_hdf5_mapping_dict):
        ...
        if self.name not in ['DiTing', 'DiTing_100Hz', 'STEAD', 'INSTANCE']:
            print('Dataset type not Supported Yet!!!')
        
        if self.name == 'DiTing' or self.name == 'DiTing_100Hz':
            self.key_str = 'key'
            self.part_str = 'part'
            self.p_str = 'p_pick'
            self.s_str = 's_pick'
        ...
        while True:
            ...
            for idx in range(0,len(self.indexes) - self.combo_num):
                ...
                if aug_choice == 'simple_shift':
                    ...
                    p_t = float(choice_line[self.p_str])
                    s_t = float(choice_line[self.s_str])
                    part = None
                    if self.has_parts:
                    ...
                    if self.name == 'DiTing':
                        ...
                        p_t = int(p_t)*2
                        s_t = int(s_t)*2
                    elif self.name == 'DiTing_100Hz':
                        key_correct = key.split('.')
                        key = key_correct[0].rjust(6,'0') + '.' + key_correct[1].ljust(4,'0')
                        p_t = int(3000.0+p_t*100.0)
                        s_t = int(3000.0+s_t*100.0)                    
                    ...
                    for choice_id in choice_ids:
                        ...
                        if self.name == 'DiTing':
                            ...
                            s_t = int(3000.0+s_t*100.0)
                        key_list.append(key)
                        P_list.append(int(p_t))
                        S_list.append(int(s_t))
