...
class RandomCropByPSIndex(object):
    ...
    def __call__(self, sample, **kwargs):
        ...
        length = wave.shape[1]
        # [Crop]
        if P_index is np.nan and S_index is np.nan:
            start = random.randint(0, length - self.window_size)
            end = start + self.window_size
        elif P_index is np.nan and S_index is not np.nan:
            end = random.randint(max(self.window_size,S_index), length)
            start = end - self.window_size
            # try:
            #     end = random.randint(max(self.window_size,S_index), length)
            #     start = end - self.window_size
            # except:
            #     print(f"window_size:{self.window_size},P_index: {P_index}, S_index: {S_index},length:{length}")
        elif S_index is np.nan and P_index is not np.nan:
            start = random.randint(0, min(P_index, max(0,length - self.window_size)))
            end = start + self.window_size
        else:
            try:
                assert P_index < S_index
            except AssertionError:
                # print(f"from:{sample['data_path']},P_index: {P_index} < S_index: {S_index}")
                start = random.randint(0, length - self.window_size)
                wave = wave[:, start:start + self.window_size]
                sample['p'] = P_index - start
                sample['s'] = S_index - start
                sample['data'] = wave
                return sample, kwargs
            ...
            if min_start <= max_start:  # when window_size >= (P_index - S_index)
                ...
                start = random.randint(max(0,min(P_index, length - self.window_size) - 1000), min(P_index, length - self.window_size))
                # try:
                #     start = random.randint(max(0,min(P_index, length - self.window_size) - 1000), min(P_index, length - self.window_size))
                # except:
                #     print(f"P_index:{P_index},length:{length},self.window_size:{self.window_size}")

            end = start + self.window_size

        sample['p'] = P_index - start
        sample['s'] = S_index - start
        sample['data'] = wave[:, start:end]
        return sample, kwargs
