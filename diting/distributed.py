...
def init_distributed_device(args):
    ...
    _setup_for_distributed(is_global_master(args))
    return device


def create_deepspeed_config(args):
    _, _, world_size = world_info_from_env()
    args.deepspeed_config = os.path.join(os.getcwd(), "config", "deepspeed_config.json")
    # default optimizer
    optim_settings = None
    ...
    with open(args.deepspeed_config, mode="w") as writer:
        ds_config = {
            ...
            "fp16": {
                "enabled": True if args.precision not in ["bf16", "fp32"] else False,
                # "auto_cast": True,
                "loss_scale": 0,
            ...
            ds_config.update(
                {
                    "zero_optimization": {
                        "stage": 1, 
                        "reduce_bucket_size": 5e8, # (todo) tuning
                        # "overlap_comm": True, # (todo) tuning
                    }
                }
            )
            ...
            ds_config.update(
                {
                    "zero_optimization": {
                    "stage": 2,
                    "contiguous_gradients": ('base' not in args.encoder_size.lower()), # should be False if model is small,
                    "overlap_comm": True,
                    "reduce_scatter": True,
