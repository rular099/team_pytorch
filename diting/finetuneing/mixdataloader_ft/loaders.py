...
import mixdataloaders.data as data
...
class JsonDataset(Dataset):
    def __init__(self, json_path, hdf5_path,transforms):
        ...
        self.HDF_file = h5pickle.File(hdf5_path, 'r')
...
class Diting1DATASET(Dataset):
    def __init__(self, csv_path, hdf5_folder_path,augmentations,sample_num):
        ...
        self.csv = pd.read_csv(csv_path, dtype={'Key': str})
        if sample_num > 0:
...
def BaseLoaders(args, is_train,dataset_class):
    input_filename = args.train_data if is_train else args.val_data 
