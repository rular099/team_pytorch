...
from .preprocess import SFTDataset
...
def train(
    ...
    for step, (x, loss_targets, metrics_targets, meta_data_jsons) in enumerate(train_loader):
        ...
        with autocast():
            ...
            if isinstance(loss_targets, (list, tuple)):
                loss = 0
                for i in range(len(loss_targets)):
                    if i > 0: # skip the dpk task
                        valid_indices = loss_targets[i] != -1
                        if valid_indices.any():
                            valid_outputs = outputs_for_loss[i][valid_indices]
                            valid_targets = loss_targets[i][valid_indices]
                            loss += args.task_loss_weight[i] * loss_fn[i](valid_outputs, valid_targets)
                    else:
                        loss += args.task_loss_weight[i] * loss_fn[i](outputs_for_loss[i], loss_targets[i])
            
            # add by lhl            
            elif isinstance(loss_targets, dict): # det_ppk_spk_dis_emg
                ...
                if isinstance(outputs_for_loss, (list, tuple)):
...
def train_worker(args, device, ds_init) -> str:
    ...
    # Loss
    loss_fn = help_builder.get_loss(args.downstream_task, args.det_weight, args.p_weight, args.s_weight, args.loss_type, args.focal_loss_gamma)
    best_loss = (
        float("inf")
    ...
    if isinstance(loss_fn, (list, tuple)):
        loss_fn = [fn.to(device) for fn in loss_fn]
    else:
