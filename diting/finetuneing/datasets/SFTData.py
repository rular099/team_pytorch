...
class SFTData(DatasetBase):
    ...
    def __init__(
        ...
        if self._mode == 'train' or self._mode == 'val':
            ...
            include_subset_names = [
                'diting1', 'DitingV2', "CSNCD-compressed", "CEA0920", "STEAD", "INSTANCE", "credit", "mlaapde", "PNW-Exotic","custom"
            ]
            for subset_name in self.subset_names.split(";"):
                ...
                if subset_name in include_subset_names:
                    self._load_hdf5_files(subset_name)
                else:
                    print(f"{subset_name} not in {include_subset_names}, please add hdf5_file manually.")
    ...
    def _load_hdf5_files(self, subset_name):
        ...
        elif subset_name == 'custom':
            self.hdf5_file = self.data_dir
    ...
    def _load_meta_data(self, filename=None) -> pd.DataFrame:
        ...
        for label in ['p_target', 's_target','Dis','Mag_value']:
    ...
    def _load_event_data(self, idx: int) -> Tuple[dict, dict]:
        ...
        if self.target_column != "":
            label_data = target_event[self.target_column]
            event[self.downstream_task] = [label_data] if pd.notnull(label_data) else [-1]
    ...
    def get_data(self,target_event):
        ...
        if data_name.startswith('diting1'):
            ...
            try:
                data = self.get_data_funcs[data_name](key)
            except KeyError:
