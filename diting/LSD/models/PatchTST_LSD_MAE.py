...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, patch_len:int, stride:int, num_patch:int, 
                 ...
                 y_range:Optional[tuple]=None, verbose:bool=False,mask_ratio=False,ff_norm=False,mask_way='random', **kwargs):
        ...
        self.backbone = PatchTSTEncoder(c_in, num_patch=num_patch, patch_len=patch_len, 
                                ...
                                pe=pe, learn_pe=learn_pe, verbose=verbose,mask_ratio=mask_ratio,ff_norm=ff_norm,norm=norm,mask_way=mask_way, **kwargs)
    ...
    def forward(self, z):                             
        ...
        if self.head_type == 'cls': # MAE finetune
            # for MAE finetune
            if isinstance(res,tuple):
                res = res[0] # for MAE:x_masked
...
class PatchTSTEncoder(nn.Module):
    def __init__(self, c_in, num_patch, patch_len, 
                 ...
                 pe='zeros', learn_pe=False, verbose=False,mask_ratio=False,ff_norm=False,mask_way='random',**kwargs):
        ...
        # encoder pos
        self.encoder_pos_embed = nn.Parameter(torch.zeros(1, num_patch, d_model), requires_grad=False)  # fixed sin-cos embedding
    ...
    def initialize_weights(self):
        torch.manual_seed(42)
        encoder_pos_embed = positional_encoding('sincos', False, self.num_patch, self.encoder_pos_embed.shape[-1])
        self.encoder_pos_embed.data.copy_(encoder_pos_embed.float().unsqueeze(0))
    ...
    def block_masking(self,x):
        ...
        def generate_unique_random(target_shape,max_num):
            ...
            if len(target_shape) == 1:
                target = torch.randperm(max_num)[:mask_num]
            else:
                target = torch.zeros(target_shape)
                for i in range(target_shape[0]):
                    target[i] = torch.randperm(max_num)[:mask_num]
            return target
        
        # get mask block idx
        offset = generate_unique_random((B,), block_size)
        k = generate_unique_random((B, num_mask_block),  N // block_size)
        block_idx = k * block_size + offset.unsqueeze(-1)
    ...
    def forward(self, x) -> Tensor:          
        ...
        u = self.dropout(u + self.encoder_pos_embed)
        ...
        if self.mask_ratio:
            ...
            if self.mask_way == 'random':
                ...
                u, mask, ids_restore, _  = self.block_masking(u)
            elif self.mask_way == 'grid':
                u, mask, ids_restore, _  = self.grid_masking(u)
...
class PatchTSTDecoder(nn.Module):
    def __init__(self,
        ...
        if predict_feature:
            self.decoder_pred = nn.Linear(decoder_dim, encoder_dim, bias=True) # decoder to target_encoder feature
        else:
...
def PatchTSTEncoder_base_vit_feature(mask_ratio,
                         input_length=10000, 
                         head_type='all',
                         pre_norm=True,
                         ff_norm=False,
...
def PatchTSTEncoder_small_vit(mask_ratio,
                         input_length=10000, 
                         head_type='all',
                         pre_norm=True,
                         ff_norm=False,
...
def PatchTSTEncoder_base_blockmask(mask_ratio,
                         input_length=10000, 
                         head_type='all',
                         pre_norm=True,
                         ff_norm=False,
...
def PatchTSTEncoder_base_gridmask(mask_ratio,
                         input_length=10000, 
                         head_type='all',
                         pre_norm=True,
                         ff_norm=False,
...
def PatchTSTDecoder_small_vit(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
    ...
    return PatchTSTDecoder(
                ...
                # encoder
                encoder_dim=encoder_dim, # TODO
                # decoder
                decoder_dim=192, # TODO
                decoder_depth=8, # TODO
                decoder_num_heads=6,
                mlp_ratio=4,
                # norm
...
def PatchTSTEncoder_tiny(mask_ratio,
                         input_length=10000, 
                         head_type='all',
                         pre_norm=True,
                         ff_norm=False,
...
def PatchTSTDecoder_tiny_96(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_tiny_144(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_tiny_192(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_tiny_384(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
# jepa
def PatchTSTDecoder_common_vit_feature(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth1_dim512(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth2_dim512(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth4_dim512(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth8_dim512(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth12_dim512(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth8_dim128(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth8_dim256(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth8_dim384(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth8_dim768(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
def PatchTSTDecoder_depth8_dim1024(encoder_dim,
                           input_length=10000,
                           pre_norm=True,
                           ff_norm=False,):
    stride = 50
...
if __name__ == "__main__":
    model = PatchTSTDecoder(                
                ...
                ff_norm=True, # default=False
                pre_norm=True # default=False
                )
    input = torch.randn(8, 51, 192) # x: [bs x (kept_num + 1) x encoder_dim]
    ids_restore = torch.arange(200).repeat(8,1).reshape(8,-1)
    out = model(input,ids_restore)  # x: [bs x patch_num x nvars * patch_len] = [8 x 200 x 3 * 50]
