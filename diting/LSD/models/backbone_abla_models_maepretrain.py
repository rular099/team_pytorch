...
class TSTEncoder(nn.Module):
    ...
    def forward(self, src:Tensor, position_ids=None):
        """
...
class TSTEncoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, store_attn,
                norm, attn_dropout, dropout, bias, pre_norm, xattn, config, n_layers, dec_module):
        super().__init__()
    ...
    def forward(self, hidden_states:Tensor, position_ids=None):
        ...
        hidden_states = residual + self.dropout_attn(hidden_states)
        ...
        ## Add & Norm (Residual Connection Scaling)
        hidden_states = residual + self.dropout_ffn(hidden_states)
