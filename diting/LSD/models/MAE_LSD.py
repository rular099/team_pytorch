...
class MaskedAutoencoderViT(nn.Module):
    ...
    """
    def __init__(self, img_size=10000, patch_size=50, in_chans=3,mask_ratio=None,
...
def mae_vit_large_patch30_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=30, embed_dim=1024, depth=24, num_heads=16,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def mae_vit_huge_patch30_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=30, embed_dim=1280, depth=32, num_heads=16,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),norm_pix_loss=False, **kwargs)
    return model

def mae_vit_base_patch50_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
...
mae_vit_base_patch30 = mae_vit_base_patch30_dec512d8b  # decoder: 512 dim, 8 blocks
mae_vit_large_patch30 = mae_vit_large_patch30_dec512d8b  # decoder: 512 dim, 8 blocks
mae_vit_huge_patch30 = mae_vit_huge_patch30_dec512d8b  # decoder: 512 dim, 8 blocks

mae_vit_base_patch50 = mae_vit_base_patch50_dec512d8b  # decoder: 512 dim, 8 blocks

if __name__ == "__main__":
    model = mae_vit_base_patch50_dec512d8b(mask_ratio=0.75)
    ...
    # output = model(input)
    # make_dot(output, params=dict(model.named_parameters())).render("model_graph_vit", format="png")